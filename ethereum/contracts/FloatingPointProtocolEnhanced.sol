...
/**
 * @title FloatingPointProtocol - Enhanced for Institutional Use
 * @notice 增强版本: 移除严格限制，添加机构支持
 * @dev 基于原版本，优化机构级大额交易
 * 
 * 主要改进:
 * 1. 大幅提高存款上限 (100K -> 100M USDT)
 * 2. 增加批处理大小 (10 -> 100)
 * 3. 提高速率限制 (100 -> 10000 tx/hour)
 * 4. 新增机构白名单功能
 * 5. 可配置的动态限制
 * 6. VIP费率机制
 */
...
/**
 * @title FloatingPointProtocolEnhanced
 * @notice 机构级增强版本
 */
contract FloatingPointProtocolEnhanced is ReentrancyGuard, Pausable, Ownable {
    ...
    // 🔥 大幅提高限制
    uint256 public constant MAX_BATCH_SIZE = 100; // 10 -> 100
    uint256 public constant MAX_DEPOSIT = 100_000_000 * 10**6; // 100,000 -> 100,000,000 USDT
    uint256 public constant MIN_DEPOSIT = POINT_VALUE;
    
    uint256 public constant MIN_RING_SIZE = 5;
    uint256 public constant MAX_RING_SIZE = 50; // 15 -> 50
    uint256 public constant MIN_FEE_RATE = 1; // 0.01%
    uint256 public constant MAX_FEE_RATE = 500; // 5%
    uint256 public constant RATE_LIMIT_WINDOW = 1 hours;
    uint256 public constant MAX_TRANSACTIONS_PER_WINDOW = 10000; // 100 -> 10000
    uint256 public constant TREASURY_TIMELOCK = 48 hours;
    uint256 public constant MAX_DEADLINE_DURATION = 1 hours;
    uint256 public constant POINT_LOCK_DURATION = 12 seconds;
    uint256 public constant ORACLE_STALENESS_THRESHOLD = 1 hours;
    uint256 public constant MIN_MULTISIG_SIGNERS = 2;
    uint256 public constant MAX_GAS_PER_POINT = 100000;
    ...
    struct InstitutionalAccount {
        bool isWhitelisted;
        uint256 customDepositLimit; // 0 = 使用默认
        uint256 customBatchSize; // 0 = 使用默认
        uint256 depositFeeRate; // 0 = 使用默认
        uint256 withdrawalFeeRate; // 0 = 使用默认
        bool exemptFromRateLimit; // 免除速率限制
        uint256 tier; // 1=Standard, 2=Premium, 3=Institutional
    }
    ...
    // 🔥 新增: TVL保护上限（可通过多签调整）
    uint256 public maxTotalValueLocked = 50_000_000 * 10**6; // 初始50M USDT
    ...
    // ============ 新增事件 ============
    event InstitutionalAccountAdded(address indexed account, uint256 tier);
    event InstitutionalAccountUpdated(address indexed account, uint256 tier);
    event InstitutionalAccountRemoved(address indexed account);
    event MaxTVLUpdated(uint256 oldMax, uint256 newMax);
    event CustomLimitSet(address indexed account, uint256 depositLimit, uint256 batchSize);
    event VIPFeeRateSet(address indexed account, uint256 depositFee, uint256 withdrawalFee);
    ...
    modifier rateLimited() {
        // 检查是否为机构且豁免
        InstitutionalAccount memory inst = institutionalAccounts[msg.sender];
        if (inst.isWhitelisted && inst.exemptFromRateLimit) {
            _;
            return;
        }
    ...
    modifier validArrays(uint256 len, uint256 maxLen) {
        ...
        // 机构可以使用自定义批次大小
        InstitutionalAccount memory inst = institutionalAccounts[msg.sender];
        uint256 effectiveMax = (inst.isWhitelisted && inst.customBatchSize > 0) 
            ? inst.customBatchSize 
            : maxLen;
        
        if (len > effectiveMax) revert ArrayTooLarge();
    ...
    constructor(
        ...
        DOMAIN_SEPARATOR = keccak256(abi.encode(
            ...
            keccak256("FloatingPointProtocolEnhanced"),
            keccak256("2.1"),
            block.chainid,
            address(this)
        ));
     ...
     * @notice 添加机构账户
     * @param account 机构地址
     * @param tier 等级 (1=Standard, 2=Premium, 3=Institutional)
     * @param customDepositLimit 自定义存款限额 (0=使用默认)
     * @param customBatchSize 自定义批量大小 (0=使用默认)
     * @param depositFee VIP存款费率 (0=使用默认)
     * @param withdrawalFee VIP提款费率 (0=使用默认)
     * @param exemptFromRateLimit 是否豁免速率限制
     * @param actionHash 多签哈希
     */
    function addInstitutionalAccount(
        address account,
        uint256 tier,
        uint256 customDepositLimit,
        ...
        if (tier == 0 || tier > 3) revert InvalidAmount();
        
        bytes32 expectedHash = keccak256(abi.encodePacked(
            "addInstitution", account, tier, customDepositLimit, customBatchSize, depositFee, withdrawalFee, exemptFromRateLimit
        ));
        if (actionHash != expectedHash) revert InvalidSignature();
        
        institutionalAccounts[account] = InstitutionalAccount({
            isWhitelisted: true,
            customDepositLimit: customDepositLimit,
            customBatchSize: customBatchSize,
            depositFeeRate: depositFee,
            withdrawalFeeRate: withdrawalFee,
            exemptFromRateLimit: exemptFromRateLimit,
            tier: tier
        });
        
        emit InstitutionalAccountAdded(account, tier);
     ...
     * @notice 移除机构账户
     */
    function removeInstitutionalAccount(address account, bytes32 actionHash) 
        external requireMultiSig(actionHash) 
    {
        bytes32 expectedHash = keccak256(abi.encodePacked("removeInstitution", account));
        if (actionHash != expectedHash) revert InvalidSignature();
        
        delete institutionalAccounts[account];
        emit InstitutionalAccountRemoved(account);
        emit MultiSigActionExecuted(actionHash, msg.sender);
    }
     ...
     * @notice 更新TVL上限
     */
    function setMaxTVL(uint256 newMax, bytes32 actionHash) 
        external requireMultiSig(actionHash) 
    {
        bytes32 expectedHash = keccak256(abi.encodePacked("setMaxTVL", newMax));
        if (actionHash != expectedHash) revert InvalidSignature();
        
        uint256 oldMax = maxTotalValueLocked;
        maxTotalValueLocked = newMax;
        
        emit MaxTVLUpdated(oldMax, newMax);
        emit MultiSigActionExecuted(actionHash, msg.sender);
    }
        ...
        validArrays(commitments.length, MAX_BATCH_SIZE)
    {
        ...
        // 🔥 检查自定义限额或默认限额
        InstitutionalAccount memory inst = institutionalAccounts[msg.sender];
        uint256 effectiveMax = (inst.isWhitelisted && inst.customDepositLimit > 0) 
            ? inst.customDepositLimit 
            : MAX_DEPOSIT;
        
        if (totalAmount < MIN_DEPOSIT || totalAmount > effectiveMax) revert InvalidAmount();
        
        // 🔥 检查TVL上限
        if (totalDeposited + totalAmount > maxTotalValueLocked) revert TVLLimitExceeded();
        
        // 🔥 使用VIP费率或默认费率
        uint256 effectiveFeeRate = (inst.isWhitelisted && inst.depositFeeRate > 0) 
            ? inst.depositFeeRate 
            : depositFeeRate;
        
        uint256 fee = _safeFeeMul(totalAmount, effectiveFeeRate);
        uint256 netAmount;
        unchecked {
    ...
    function getInstitutionalAccount(address account) 
        external 
        view 
        returns (InstitutionalAccount memory) 
    {
        return institutionalAccounts[account];
    }
    
    function getEffectiveDepositLimit(address account) external view returns (uint256) {
        InstitutionalAccount memory inst = institutionalAccounts[account];
        return (inst.isWhitelisted && inst.customDepositLimit > 0) 
            ? inst.customDepositLimit 
            : MAX_DEPOSIT;
    }
    
    function getEffectiveFeeRates(address account) 
        external 
        view 
        returns (uint256 depositFee, uint256 withdrawalFee) 
    {
        InstitutionalAccount memory inst = institutionalAccounts[account];
        depositFee = (inst.isWhitelisted && inst.depositFeeRate > 0) 
            ? inst.depositFeeRate 
            : depositFeeRate;
        withdrawalFee = (inst.isWhitelisted && inst.withdrawalFeeRate > 0) 
            ? inst.withdrawalFeeRate 
            : withdrawalFeeRate;
    }
    
    // Note: 其他函数（privacyPayment, withdrawal等）继承自原版本
    // 这里为了简洁省略，实际部署时需要包含所有原有函数
}
